...
RUN cd ${JAVA_APP_DIR} && mvn clean install
...
# Copy .NET Core runtime from the build-stage (avoiding reinstallation)
COPY --from=build-stage /usr/share/dotnet /usr/share/dotnet

# Copy Java runtime (JRE) from the build-stage
COPY --from=build-stage /usr/lib/jvm /usr/lib/jvm

# Copy built Java and .NET Core applications from the build stage
COPY --from=build-stage ${JAVA_APP_DIR}/target/*.jar ${JAVA_APP_JAR}
COPY --from=build-stage ${NETCORE_APP_DIR}/bin/Debug/net8.0/* ${DOTNET_APP_DIR}

# Add .NET Core and Java to the PATH
ENV PATH="$PATH:/usr/share/dotnet:/usr/lib/jvm/java-17-openjdk-amd64/bin"
